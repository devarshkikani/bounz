...
///
/// Used by [RewardDropdownButton.selectedItemBuilder].
typedef RewardDropdownButtonBuilder = List<Widget> Function(
...
class _RewardDropdownMenuItemButton<T> extends StatefulWidget {
  const _RewardDropdownMenuItemButton({
    super.key,
    this.padding,
    required this.route,
  ...
  @override
  _RewardDropdownMenuItemButtonState<T> createState() =>
      _RewardDropdownMenuItemButtonState<T>();
}

class _RewardDropdownMenuItemButtonState<T>
    extends State<_RewardDropdownMenuItemButton<T>> {
  ...
  void _handleOnTap() {
    final RewardDropdownMenuItem<T> dropdownMenuItem =
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> children = <Widget>[
      for (int itemIndex = 0; itemIndex < route.items.length; ++itemIndex)
        _RewardDropdownMenuItemButton<T>(
          route: widget.route,
          padding: widget.padding,
...
class _DropdownRoutePage<T> extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // RewardDropdownButton.itemHeight is specified or RewardDropdownButton.itemHeight is null
...
class _MenuItem<T> extends SingleChildRenderObjectWidget {
  ...
  final RewardDropdownMenuItem<T>? item;
...
// The container widget for a menu item created by a [RewardDropdownButton]. It
// provides the default configuration for [RewardDropdownMenuItem]s, as well as a
// [RewardDropdownButton]'s hint and disabledHint widgets.
class _RewardDropdownMenuItemContainer extends StatelessWidget {
  /// Creates an item for a dropdown menu.
  ///
  /// The [child] argument is required.
  const _RewardDropdownMenuItemContainer({
    super.key,
...
/// An item in a menu created by a [RewardDropdownButton].
///
/// The type `T` is the type of the value the entry represents. All the entries
/// in a given menu must represent values with consistent types.
class RewardDropdownMenuItem<T> extends _RewardDropdownMenuItemContainer {
  /// Creates an item for a dropdown menu.
  ///
  /// The [child] argument is required.
  const RewardDropdownMenuItem({
    super.key,
    this.onTap,
    this.value,
    this.enabled = true,
  ...
  ///
  /// Eventually returned in a call to [RewardDropdownButton.onChanged].
  final T? value;
...
/// An inherited widget that causes any descendant [RewardDropdownButton]
/// widgets to not include their regular underline.
///
/// This is used by [DataTable] to remove the underline from any
/// [RewardDropdownButton] widgets placed within material data tables, as
/// required by the Material Design specification.
class RewardDropdownButtonHideUnderline extends InheritedWidget {
  /// Creates a [RewardDropdownButtonHideUnderline]. A non-null [child] must
  /// be given.
  const RewardDropdownButtonHideUnderline({
    super.key,
    required super.child,
  });

  /// Returns whether the underline of [RewardDropdownButton] widgets should
  /// be hidden.
  ...
  @override
  bool updateShouldNotify(RewardDropdownButtonHideUnderline oldWidget) => false;
}
...
/// Typically, an enum is used. Each [RewardDropdownMenuItem] in [items] must be
...
///
/// {@tool dartpad}
/// This sample shows a [RewardDropdownButton] with a large arrow icon,
...
/// See also:
///
///  * [RewardDropdownButtonFormField], which integrates with the [Form] widget.
///  * [RewardDropdownMenuItem], the class used to represent the [items].
///  * [RewardDropdownButtonHideUnderline], which prevents its descendant dropdown buttons
///    from displaying their underlines.
///  * [ElevatedButton], [TextButton], ordinary buttons that trigger a single action.
///  * <https://material.io/design/components/menus.html#dropdown-menu>
class RewardDropdownButton<T> extends StatefulWidget {
  /// Creates a dropdown button.
  ///
  /// The [items] must have distinct values. If [value] isn't null then it
  /// must be equal to one of the [RewardDropdownMenuItem] values. If [items] or
  ...
  RewardDropdownButton({
    ...
    // RewardDropdownButtonFormField.
  })  : assert(
          items == null ||
              items.isEmpty ||
              value == null ||
              items.where((RewardDropdownMenuItem<T> item) {
                    return item.value == value;
                  }).length ==
                  1,
          "There should be exactly one item with [RewardDropdownButton]'s value: "
          '$value. \n'
          'Either zero or 2 or more [RewardDropdownMenuItem]s were detected '
          'with the same value',
        ),
  ...
  RewardDropdownButton._formField({
    super.key,
    required this.items,
    this.selectedItemBuilder,
    this.value,
    ...
    required bool isFocused,
          ...
          items == null ||
              ...
              items.where((RewardDropdownMenuItem<T> item) {
                  ...
                  1,
          "There should be exactly one item with [RewardDropdownButtonFormField]'s value: "
          '$value. \n'
          'Either zero or 2 or more [RewardDropdownMenuItem]s were detected '
          'with the same value',
        ),
  ...
  final List<RewardDropdownMenuItem<T>>? items;

  /// The value of the currently selected [RewardDropdownMenuItem].
  ///
  /// If [value] is null and the button is enabled, [hint] will be displayed
  /// if it is non-null.
  ///
  ...
  ///
  /// If the [onChanged] callback is null or the list of [RewardDropdownButton.items]
  /// is null then the dropdown button will be disabled, i.e. its arrow will be
  /// displayed in grey and it will not respond to input. A disabled button
  /// will display the [RewardDropdownButton.disabledHint] widget if it is non-null.
  /// If [RewardDropdownButton.disabledHint] is also null but [RewardDropdownButton.hint] is
  /// non-null, [RewardDropdownButton.hint] will instead be displayed.
  /// {@endtemplate}
  ...
  /// [RewardDropdownMenuItem]s in [items].
  ///
  /// When a [RewardDropdownMenuItem] is selected, the widget that will be displayed
  /// from the list corresponds to the [RewardDropdownMenuItem] of the same index
  /// in [items].
  ///
  /// {@tool dartpad}
  /// This sample shows a `RewardDropdownButton` with a button with [Text] that
  /// corresponds to but is unique from [RewardDropdownMenuItem].
  ///
  /// ** See code in examples/api/lib/material/dropdown/dropdown_button.selected_item_builder.0.dart **
  /// {@end-tool}
  ///
  /// If this callback is null, the [RewardDropdownMenuItem] from [items]
  /// that matches [value] will be displayed.
  final RewardDropdownButtonBuilder? selectedItemBuilder;
  ...
  ///
  /// {@tool dartpad}
  /// This sample shows a `RewardDropdownButton` with a dropdown button text style
  ...
  @override
  State<RewardDropdownButton<T>> createState() =>
      _RewardDropdownButtonState<T>();
}

class _RewardDropdownButtonState<T> extends State<RewardDropdownButton<T>>
  ...
  @override
  void didUpdateWidget(RewardDropdownButton<T> oldWidget) {
  ...
  void _updateSelectedIndex() {
    if (widget.items == null ||
        ...
        (widget.value == null &&
            widget.items!
                .where((RewardDropdownMenuItem<T> item) =>
    ...
    assert(widget.items!
            .where(
                (RewardDropdownMenuItem<T> item) => item.value == widget.value)
            .length ==
        1);
  ...
  void _handleTap() {
    ...
    final Rect itemRect = itemBox.localToGlobal(const Offset(-25, -3),
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.hint != null || (!_enabled && widget.disabledHint != null)) {
      ...
      items.add(DefaultTextStyle(
        ...
        child: IgnorePointer(
          ignoring: false,
          child: _RewardDropdownMenuItemContainer(
    ...
    if (!RewardDropdownButtonHideUnderline.at(context)) {
      final double bottom =
...
/// A [FormField] that contains a [RewardDropdownButton].
///
/// This is a convenience widget that wraps a [RewardDropdownButton] widget in a
/// [FormField].
///
...
/// See also:
///
///  * [RewardDropdownButton], which is the underlying text field without the [Form]
///    integration.
class RewardDropdownButtonFormField<T> extends FormField<T> {
  /// Creates a [RewardDropdownButton] widget that is a [FormField], wrapped in an
  /// [InputDecorator].
  ///
  ...
  RewardDropdownButtonFormField({
    super.key,
    required List<RewardDropdownMenuItem<T>>? items,
    RewardDropdownButtonBuilder? selectedItemBuilder,
    T? value,
    Widget? hint,
    Widget? disabledHint,
    required this.onChanged,
    ...
    // RewardDropdownButton.
        ...
        super(
          ...
          builder: (FormFieldState<T> field) {
            final _RewardDropdownButtonFormFieldState<T> state =
                field as _RewardDropdownButtonFormFieldState<T>;
            ...
            return Focus(
              ...
              child: Builder(builder: (BuildContext context) {
                return RewardDropdownButtonHideUnderline(
                  child: RewardDropdownButton<T>._formField(
                    items: items,
  ...
  @override
  FormFieldState<T> createState() => _RewardDropdownButtonFormFieldState<T>();
}

class _RewardDropdownButtonFormFieldState<T> extends FormFieldState<T> {
  @override
  void didChange(T? value) {
    super.didChange(value);
    final RewardDropdownButtonFormField<T> dropdownButtonFormField =
        widget as RewardDropdownButtonFormField<T>;
